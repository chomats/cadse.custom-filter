...
//			mi.setImage(JavaPluginImages.get(JavaPluginImages.IMG_OBJS_JAVA_WORKING_SET));
	...
	public void fillViewMenu(IMenuManager viewMenu) {
		...
		if (fMenuManager != null)
			throw new IllegalStateException("FilleViewMenu allready called");
		 ...
		 */
		fMenuManager.add(new Separator("filters")); //$NON-NLS-1$
		fMenuManager.add(new GroupMarker(RECENT_FILTERS_GROUP_NAME));
		fMenuManager.add(new ShowFilterDialogAction());
	...
	private void initializeWithPluginContributions() {
		...
		for (int i= 0; i < filterDescs.length; i++) {
			...
			if (fEnabledFilterIds.containsKey(id))
				CustomePlugin.logErrorMessage("WARNING: Duplicate id for extension-point \"org.eclipse.jdt.ui.javaElementFilters\""); //$NON-NLS-1$
	...
	private void initializeWithViewDefaults() {
		...
		IPreferenceStore store= CustomePlugin.getDefault().getPreferenceStore();
